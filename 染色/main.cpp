...
public:
    ...
    Node operator + (const Node& other)
    {
        ...
        if (r != -1 && other.l != -1 && r == other.l)
            t = 1;
        return { cnt + other.cnt - t, l, other.r };
    }
};
...
public:
    ...
    SegTree(int newSize, const vector<int>& arr) : size(newSize), seg(4 * newSize, {0,-1,-1}), lazy(4 * newSize), range(4 * newSize)
    {
        build(0, 1, size, arr);
    }
    SegTree(int newSize) : size(newSize), seg(4 * newSize, { 0,-1,-1 }), lazy(4 * newSize), range(4 * newSize)
    {
        build(0, 1, size, vector<int>(size + 1));
    }
    void PushDown(int x)
    {
    ...
    Node GetRange(int x, int l, int r)
    {
        if (l <= range[x].first && r >= range[x].second)
        {
            return seg[x];
        ...
        PushDown(x);
        Node lNode =  GetRange((x << 1) + 1, l, r), rNode = GetRange((x << 1) + 2, l, r);
        if (lNode.cnt == 0)
            return rNode;
        if (rNode.cnt == 0)
            return lNode;
...
Node lcaQ(SegTree & seg,int a, int b)
{
    ...
    while (top[a] != top[b])
    {
        if (depth[top[a]] < depth[top[b]])
            swap(a, b);
        Node t = seg.GetRange(0, tAtN[top[a]], tAtN[a]);
        cur = t  + cur;
        a = chain.parent[top[a]];
    }
    if (depth[a] > depth[b])
        swap(a, b);
    Node t = seg.GetRange(0, tAtN[a], tAtN[b]);
    cur = t + cur;
    return cur;
}

void lcaC(SegTree &seg,int a, int b, int v)
{
    auto& top = chain.top;
    auto& depth = chain.depth;
    auto& tAtN = chain.timeAtNode;
    while (top[a] != top[b])
    {
        if (depth[top[a]] < depth[top[b]])
            swap(a, b);
        seg.ChangeRange(0, tAtN[top[a]], tAtN[a], v);
        a = chain.parent[top[a]];
    }
    if (depth[a] > depth[b])
        swap(a, b);
    seg.ChangeRange(0, tAtN[a], tAtN[b], v);
}
int main()
{
    int n = read(), m = read();
    
    vector<int> color(n + 1), color2(n + 1);
    for (int i = 1; i <= n; ++i)
        color[i] = read();
    for (int i = 1; i < n; ++i)
    {
    ...
    while (m--)
    {
        ...
        if (op == 'Q')
        {
            int a = read(), b = read();
            int l = lca(a, b);
            Node t1 = lcaQ(seg, a, l), t2 = lcaQ(seg, b, l);
            swap(t1.l, t1.r);
            write((t1 + t2).cnt);
            putchar('\n');
        }
